...
bool Cal3DS2_k3_Base::equals(const Cal3DS2_k3_Base& K, double tol) const
{
    ...
    return Cal3::equals(*base, tol) && std::fabs(k1_ - K.k1_) < tol && std::fabs(k2_ - K.k2_) < tol &&
           std::fabs(p1_ - K.p1_) < tol && std::fabs(p2_ - K.p2_) < tol && std::fabs(k3_ - K.k3_) < tol;
}

/* ************************************************************************* */
Eigen::Matrix<double, 2, 10> D2dcalibration(
    double x,
    double y,
    double xx,
    double yy,
...
/* ************************************************************************* */
Point2 Cal3DS2_k3_Base::uncalibrate(const Point2& p, OptionalJacobian<2, 10> Dcal, OptionalJacobian<2, 2> Dp) const
{
    //  r² = x² + y²;
...
/* ************************************************************************* */
Point2 Cal3DS2_k3_Base::calibrate(const Point2& pi, OptionalJacobian<2, 10> Dcal, OptionalJacobian<2, 2> Dp) const
{
    // Use the following fixed point iteration to invert the radial distortion.
    // pn_{t+1} = (inv(K)*pi - dp(pn_{t})) / g(pn_{t})

    const Point2 invKPi((1 / fx_) * (pi.x() - u0_ - (s_ / fy_) * (pi.y() - v0_)), (1 / fy_) * (pi.y() - v0_));
