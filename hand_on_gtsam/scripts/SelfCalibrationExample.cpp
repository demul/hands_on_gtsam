...
int main(int argc, char* argv[])
{
    ...
    for (size_t i = 0; i < poses.size(); ++i)
    {
        for (size_t j = 0; j < points.size(); ++j)
        {
            ...
            graph.emplace_shared<GeneralSFMFactor2<Cal3_S2> >(
                measurement, measurementNoise, Symbol('x', i), Symbol('l', j), Symbol('K', 0));
        }
    }
    ...
    for (size_t j = 0; j < points.size(); ++j)
        graph.addPrior(Symbol('l', j), points[j],
    ...
    for (size_t i = 0; i < poses.size(); ++i)
        initialEstimate.insert(
            Symbol('x', i), poses[i].compose(Pose3(Rot3::Rodrigues(-0.1, 0.2, 0.25), Point3(0.05, -0.10, 0.20))));
    for (size_t j = 0; j < points.size(); ++j)
